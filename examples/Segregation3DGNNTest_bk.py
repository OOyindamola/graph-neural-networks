...
create_folder = True
lodDir ='SegregationGNN_NStates21_-021-20230215020229DlAggGNN_K3_clipping__DAGGER_'
label_name = 'Interval2680'
startEpoch = 0
...
alpha = [1,2,5]
duration = [20,15,10]
# groups_ = [5, 3,5, 3 , 7, 5, 2, 5, 10, 5]
# agents = [100, 21,20, 21, 21, 30, 50, 50, 50,100]
groups_ = ['5']
agents = [ '30']
...
for ik in range(1):
	...
	# else:
	dAA    = 5.0  #% distance among same types.
	dAB    = 3.0 # % distance among distinct types.
	exp ="Aggregation3D"
	for i in range(len(agents)):
		...
		#\\\ Training algorithm
		trainer = training.TrainerSegregate3D
		...
		if doDlAggGNN:
			...
			# Readout layer: local linear combination of features
			hParamsDAGNN1Ly['dimReadout'] = [64,3] # Dimension of the fully connected
				# layers after the GCN layers (map); this fully connected layer
				# is applied only at each node, without any further exchanges nor
				# considering all nodes at once, making the architecture entirely
				# local.
		...
		for realization in range(nRealizations):
			...
			initPos = np.load('/home/oyindamola/Research/Current_GNN_Research/graph-neural-networks/alegnn/utils/Testsegregate3D_initPos_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+'_Samples42.npy',allow_pickle=True)
			initVel = np.load('/home/oyindamola/Research/Current_GNN_Research/graph-neural-networks/alegnn/utils/Testsegregate3D_initVel_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+'_Samples42.npy',allow_pickle=True)
			initData = (initPos, initVel)

			print("")
			#   Generate the dataset
			data = dataTools.Segregation3D(
						# Structure
						nAgents,
						nstates,
						alpha,
			...
			initPosValid = data.getData('initPos','test')[start:stop]
			initVelValid = data.getData('initVel','test')[start:stop]

			initPosValid[:, 2, :] = initPosValid[:, 2, :] + 4

			print(":", initPosValid[:, 2, :])

			# print("initPosValid:", initPosValid.shape)

			np.save('./Testsegregate3D_initPos_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(1), initPosValid)
			np.save('./Testsegregate3D_initVel_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(1), initVelValid)
			...
			for gl in range(len(gl__)):
				gl_ = gl__[gl]
				if gl_=='True':
					type_alg = 'global'
					posTestValid = data.posAll[start+2:stop+2]
					velTestValid = data.velAll[start+2:stop+2]
				elif gl_ =='learner':
					type_alg = 'learner'
					posTestValid, velTestValid, _, _, graphTestValid = data.computeTrajectory(
							initPosValid, initVelValid, data.duration,
							archit = thisArchit, doPrint = False, clipping=True)
					np.save('./Testsegregate3D_TrajPos_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(1), posTestValid)
					np.save('./Testsegregate3D_TrajVel_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(1), velTestValid)


				else:
					type_alg = 'local'
					posTestValid = data.localposAll[start:stop]
					velTestValid = data.localvelAll[start:stop]
				...
				# print("g: ", posTestValid)

				# print("g: ", gl_,  posTestValid)


				# np.save('./Testsegregate3D_FinalPos_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(stop), posTestValid)
				# np.save('./Testsegregate3D_FinalVel_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(stop), velTestValid)
				#
				# np.save('./Testsegregate3D_initPos_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(stop), initPosValid)
				# np.save('./Testsegregate3D_initVel_Agent'+str(nAgents)+'_GR'+str(GROUPS)+'_CR'+str(commRadius)+"_Samples"+str(stop), initVelValid)
				...
				for ii in range(t_len):
					log_dir = modelCreated.saveDir+"/"+type_alg+"/"+str(nAgents)+"_"+str(GROUPS)+"_"+exp+'_'+today+"/"+str(ii)
					...
					data.make_gif(posTestValid_[0], saveDir)
