...
class Trainer:
	...
	def __init__(self, model, data, nEpochs, startEpoch, batchSize, **kwargs):
		...
		self.trainingOptions['startEpoch'] = startEpoch
	...
	def train(self):
		...
		while epoch < nEpochs \
			...
			print("randomPermutation: ", randomPermutation)
			print("idxEpoch: ", idxEpoch)
		...
		for s in range(nTrain):
			...
			if np.random.binomial(1, chooseExpertProb) == 1:
				...
				SDAGaux = self.data.computeCommunicationGraph(
						posDAG, self.data.commRadius,  self.data.normalizeGraph, doPrint = False)
			...
			for t in range(1,xTrainAll.shape[1]):
				...
				thisGraph = self.data.computeCommunicationGraph(
					posDAG[s:s+1,t:t+1,:,:], self.data.commRadius,
					 self.data.normalizeGraph, doPrint = False)
...
class TrainerSegregate(Trainer):
	"""
	Trainer: trains segr models, following the appropriate evaluation of
		the cost, and has options for different DAGger alternatives
	...
	def __init__(self, model, data, nEpochs, startEpoch, batchSize, **kwargs):
		...
		# Initialize supraclass
		super().__init__(model, data, nEpochs,startEpoch, batchSize, **kwargs)
		...
		self.logger_ = Logger(self.model.saveDir)
	...
	def train(self):
		...
		assert 'nEpochs' in self.trainingOptions.keys()
		nEpochs = self.trainingOptions['nEpochs']
		assert 'startEpoch' in self.trainingOptions.keys()
		startEpoch = self.trainingOptions['startEpoch']
		assert 'nBatches' in self.trainingOptions.keys()
		nBatches = self.trainingOptions['nBatches']
		assert 'graphNo' in self.trainingOptions.keys()
		graphNo = self.trainingOptions['graphNo']
		...
		assert 'doDAGger' in self.trainingOptions.keys()
		doDAGger = self.trainingOptions['doDAGger']
		# assert 'clipping' in self.trainingOptions.keys()
		# clipping = self.trainingOptions['clipping']
		...
		thisOptim = self.model.optim
		thisDevice = self.model.device
		const_ = self.data.const
		alpha = self.data.alpha
		clipping = self.data.clipping


		# print(" self.normalizeGraph:",  self.data.normalizeGraph)
		# Learning rate scheduler:
		if doLearningRateDecay:
			learningRateScheduler = torch.optim.lr_scheduler.StepLR(self.optim,
					learningRateDecayPeriod, learningRateDecayRate)
		...
		# Initialize counters (since we give the possibility of early stopping,
		# we had to drop the 'for' and use a 'while' instead):
		epoch = 0 +startEpoch # epoch counter
		lagCount = 0 # lag counter for early stopping
		...
		while epoch < nEpochs \
			...
			while batch < nBatches \
				...
				# Compute loss
				lossValueTrain = thisLoss(yHatTrain, yTrain)
				# print("lossValueTrain:", lossValueTrain)
				...
				if (epoch * nBatches + batch) % validationInterval == 0:
					...
					# Compute trajectories
					posTestValid, velTestValid, _, _, graphTestValid = self.data.computeTrajectory(
							initPosValid, initVelValid, self.data.duration,
							archit = thisArchit, doPrint = False, clipping=clipping)

					# print("posTestValid: ", posTestValid.shape)

					# Compute evaluation
					accValid, stats = self.data.evaluate(pos=posTestValid, \
					vel = velTestValid)

					if (epoch * nBatches + batch) % (validationInterval) == 0:
						self.model.save(label = 'Interval'+str((epoch * nBatches + batch)))

					if (epoch * nBatches + batch) % (validationInterval*5) == 0:
						# self.model.save(label = 'Interval'+str((epoch * nBatches + batch)))

						self.data.saveVideo(self.model.saveDir, # Where to save them
				                        ...
				                        videoSpeed = 5,
										videoNum= str((epoch * nBatches + batch))) # Change speed of animation
					...
					self.perform_logging(self.logs, self.logger_, epoch, stats, accValid, lossValueTrain.item(), True)


					# Print:
					if doPrint:
						print("\t(E: %2d, B: %3d, A: %2d, N: %2d, DAA: %2d, DAB: %2d, V: %2d) %8.4f - %6.4fs" % (
								epoch+1, batch+1, area, n_clusters,
								AvgdAA, AvgdAB, VM,
								accValid,
								timeElapsed), end = ' ')
						print("[VALIDATION", end = '')
					...
					# No previous best option, so let's record the first trial
					# as the best option
					if (epoch == 0 or epoch == startEpoch) and batch == 0:
						bestScore = accValid
						bestEpoch, bestBatch = epoch, batch
						# Save this model as the best (so far)
						self.model.save(label = 'Best')
	...
	def randomEpochDAGger(self, epoch, xTrainOrig, yTrainOrig, StrainOrig,
						  initPosTrainAll, initVelTrainAll, const_, alpha,
						  clipping):
		...
		assert 'probExpert' in self.trainingOptions.keys()
		probExpert = self.trainingOptions['probExpert']
		nTrain = xTrainOrig.shape[0]
			...
			if np.random.binomial(1, chooseExpertProb) == 1:
				...
				if posDAG.shape[1] > maxTimeSamples:
					...
					for t in range(posDAG.shape[1]):
						...
						offset = self.data.goal - posDAG[:, t, :, :]
						distance = np.linalg.norm(offset[:], axis=(1,2))
						target_speed = self.data.accelMax * np.minimum(1, distance/20 )
						target_velocity = target_speed.reshape(1,1,1) * np.divide(offset,distance.reshape(1,1,1))

						goal_accel = target_velocity #- velDAG[:, t, :, :]
				...
				SDAGaux = self.data.computeCommunicationGraph(
						posDAG, self.data.commRadius,  self.data.normalizeGraph, doPrint = False)
				xDAGaux = self.data.computeStates(posDAG, velDAG, SDAGaux, const_,
											 doPrint = False)
	...
	def fixedBatchDAGger(self, initPosTrain, initVelTrain, const_, alpha,
						clipping):
		# print("===========fixed dagger segregate==========")
		# The 'fixedBatch' option, doubles the batch samples
		# by considering the same initial velocities and
		...
		graphDAG = self.data.computeCommunicationGraph(posPol,
													   self.data.commRadius,
														self.data.normalizeGraph,
													   doPrint = False)
		xDAG = self.data.computeStates(posPol, velPol, graphDAG, const_,
									   doPrint = False)
	...
	def perform_logging(self, logs, logger, itr, stats, accValid, lossValueTrain, print_data=True):
		if logs is None:
			logs = OrderedDict()

		logs["Iteration"] = itr
		logs["lossValueTrain"] = lossValueTrain
		logs["Cost"] = accValid
		# logs["TrainReturn/Average"] = np.mean(train_returns)
		# logs["TrainReturn/Std"] = np.std(train_returns)
		# logs["TrainReturn/Max"] = np.max(train_returns)
		# logs["TrainReturn/Min"] = np.min(train_returns)
...
'''========================================'''
class TrainerSegregate3D(Trainer):
	"""
	Trainer: trains segr models, following the appropriate evaluation of
		the cost, and has options for different DAGger alternatives
		...
		while epoch < nEpochs \
			...
			while batch < nBatches \
				...
				if doDAGger and (batch > 0 or epoch > 0)\
					...
					Strain = np.concatenate((Strain, SDAG), axis = 0)
					yTrain = np.concatenate((yTrain, yDAG), axis = 0)
					initVelTrain = np.tile(initVelTrain, (3,1,1))
				#\\\
				#\\\ Finished DAGGER
				#\\\\\\\\\\\\\\\\\\\
			...
			if np.random.binomial(1, chooseExpertProb) == 1:
				...
				if posDAG.shape[1] > maxTimeSamples:
					...
					yDAGaux = \
							...
							ijDiffPos * ( const_ * ijDiffPosInv -
							ijDistSqInv +  const_ * (ijDistSqInv ** 2) ),
									   axis = 4)-velDAG + self.data.alpha*goal_accel
