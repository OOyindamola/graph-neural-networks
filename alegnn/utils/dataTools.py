...
plt.rcParams['animation.ffmpeg_path'] = '/home/oyindamola/anaconda3/envs/swarm-rl/bin/ffmpeg' # Add the path of ffmpeg here!!
...
from alegnn.utils.metric  import *
...
class Flocking(_data):
	...
	Input:
		...
		initVelValue (float): maximum initial velocity (in meters/seconds,
			default: 3.)
		initMinDist (float): miniFmum initial diFFstance between agents (in
			meters, default: 0.1)
		...
		for b in range(nBatches):
			...
			if tSamples > maxTimeSamples:
				...
				for t in range(tSamples):
					...
					# Apply the Kernel
					if kernelType == 'gaussian':
						graphMatrixTime = distSq #np.exp(-kernelScale * distSq)
					else:
	...
	def computeTrajectory(self, initPos, initVel, duration, **kwargs):
		...
		print("kjjj: ", batchSize, tSamples, 2, nAgents)
	...
	def computeInitialPositions(self, nAgents, nSamples, commRadius,
		...
		# And concatenate them
		velBias = np.concatenate((xVelBias, yVelBias)).reshape((nSamples,3,1))
...
# class Segregation(_data):
# 	"""
# 	Segregation: Creates synthetic trajectories for the problem of coordinating
# 		a team of robots to fly together while avoiding collisions. See the
# 		following  paper for details
#
# 		E. Tolstaya, F. Gama, J. Paulos, G. Pappas, V. Kumar, and A. Ribeiro,
...
# 	"""
#
# 	def __init__(self, nAgents,nstates, alpha, dAA, dAB, GROUPS, commRadius,
# 				repelDist, clipping,
# 				 nTrain, nValid, nTest,
# 				 duration, samplingTime,
# 				 initGeometry = 'circular',initVelValue = 3.,initMinDist = 0.1,
# 				 accelMax = 10.,
# 				 normalizeGraph = False, doPrint = True,
# 				 dataType = np.float64, device = 'cpu'):
#
# 		# Initialize parent class
# 		super().__init__()
...
# 		self.dAA    = dAA;  #% distance among same types.
# 		self.dAB    = dAB; # % distance among distinct types.
# 		self.GROUPS = GROUPS;    #% number of groups.
# 		# self.WORLD computeSta = 30;   #% world size.
...
# 		self.nAgents = nAgents
#
# 		self.clustermetric = ClusterMetric(self.GROUPS, self.nAgents)
# 		self.n_clustermetric = NClusterMetric(self.GROUPS, self.nAgents, self.dAA)
# 		self.areametric = AverageMetric(self.GROUPS, self.nAgents)
#
#
# 		i,j = np.meshgrid(np.arange(1,self.nAgents+1), np.arange(1,self.nAgents+1), indexing='xy')
# 		gpr = self.GROUPS / self.nAgents;
# 		self.AA  = (np.floor(gpr*(i-1)) == np.floor(gpr*(j-1)));
# 		self.AB  = (np.floor(gpr*(i-1)) != np.floor(gpr*(j-1)));
# 		del i, j
# 		self.const = self.dAA * self.AA + self.dAB * self.AB
# 		self.const  = np.expand_dims(self.const , 0)
# 		self.const  = np.expand_dims(self.const , 0)
#
#
#
...
# 										  yMaxInitVel = self.initVelValue
# 															  )
# 		# print("initPosAll: ", initPosAll.shape)
# 		# print("initVelAll: ", len(initVelAll), len(initVelAll[0]))
# 		#   Once we have all positions and velocities, we will need to split
# 		#   them in the corresponding datasets (train, valid and test)
# 		self.initPos = {}
# 		self.initVel = {}
...
# 		posAll, velAll, accelAll = self.computeOptimalTrajectory(
# 										initPosAll, initVelAll, self.duration,
# 										self.samplingTime, self.repelDist, self.alpha,
# 										self.const,
# 										accelMax = self.accelMax)
#
#
...
# 													  self.normalizeGraph)
#
# 		# print("commGraphAll: ", commGraphAll.shape)
# 		self.commGraph = {}
#
#
#
...
#
# 		# Compute the states
# 		stateAll = self.computeStates(posAll, velAll, commGraphAll, self.const)
#
# 		if self.doPrint:
# 			print("OK", flush = True)
# 			# Erase the label first, then print it
# 			print("\tComputing the local traj...", end = ' ', flush = True)
#
# 		#compute local stateTestBest
#
# 		localposAll, localvelAll, _ = self.computeLocalOptimalTrajectory(
# 										initPosAll, initVelAll, self.duration,
# 										self.samplingTime, self.repelDist, self.alpha,
# 										self.const, commGraphAll,
# 										accelMax = self.accelMax)
#
# 		self.state = {}
#
...
# 		self.state['train'] = stateAll[0:self.nTrain]
#
# 		self.local_pos['train'] = localposAll[0:self.nTrain]
# 		self.local_vel['train'] = localvelAll[0:self.nTrain]
# 		#   Validation set
# 		startSample = self.nTrain
# 		endSample = self.nTrain + self.nValid
# 		self.samples['valid']['signals']=stateAll[startSample:endSample].copy()
...
# 		self.commGraph['valid'] = commGraphAll[startSample:endSample]
# 		self.state['valid'] = stateAll[startSample:endSample]
# 		self.local_pos['valid'] = localposAll[startSample:endSample]
# 		self.local_vel['valid'] = localvelAll[startSample:endSample]
#
#
# 		#   Testing set
# 		startSample = self.nTrain + self.nValid
...
# 		self.state['test'] = stateAll[startSample:endSample]
#
# 		self.local_pos['test'] = localposAll[startSample:endSample]
# 		self.local_vel['test'] = localvelAll[startSample:endSample]
#
#
# 		# self.localposAll = localposAll[0:self.nTrain]
# 		# self.localvelAll = localvelAll[0:self.nTrain]
# 		#
# 		# self.localposValid = localposAll[0:self.nTrain]
# 		# self.localvelValid = localvelAll[0:self.nTrain]
#
# 		# Change data to specified type and device
# 		self.astype(self.dataType)
# 		self.to(self.device)
...
# 		pass
#
# 	def computeStates(self, pos, vel, graphMatrix, const_,**kwargs):
#
# 		# We get the following inputs.
# 		# positions: nSamples x tSamples x 2 x nAgents
# 		# velocities: nSamples x tSamples x 2 x nAgents
...
#
# 		# Create the output state variable
# 		state = np.zeros((nSamples, tSamples, self.nstates, nAgents))
#
# 		# print("Statedffff", state.shape)
#
# 		for b in range(nBatches):
#
# 			# Pick the batch elements
...
# 					#   posDiff: batchSize[b] x 2 x nAgents x nAgents
# 					#   posDistSq: batchSize[b] x nAgents x nAgents
# 					ijDiffVel, _ = self.computeDifferences(velBatch[:,t,:,:])
# 					#   velDiff: batchSize[b] x 2 x nAgents x nAgents
#
# 					# Next, we need to get ride of all those places where there are
# 					# no neighborhoods. That is given by the nonzero elements of the
# 					# graph matrix.
# 					# print("graphMatrixBatch[:,t,:,:]: ", graphMatrixBatch[:,t,:,:])
#
#
# 					graphMatrixTime = self.computeCommunicationGraph(np.expand_dims(posBatch[:,t,:,:], 1), self.commRadius, self.normalizeGraph,
# 				   doPrint = False)[:,0,:,:]
#
# 					# graphMatrixTime = (np.abs(graphMatrixBatch[:,t,:,:])\
# 					# 										   >zeroTolerance)\
...
# 					# We also need to invert the squares of the distances
# 					ijDistSqInv = invertTensorEW(ijDistSq)
# 					# print("posDistSqInv B: ", posDistSqInv.shape)
# 					#   posDistSqInv: batchSize[b] x nAgents x nAgents
#
# 					# Now we add the extra dimensions so that all the
# 					# multiplications are adequate
...
# 					ijDistSqInv =np.expand_dims(ijDistSqInv,1) \
# 															  * graphMatrixTime
# 					ijDiffVel = ijDiffVel * graphMatrixTime
# 					ijDiffPos = ijDiffPos * graphMatrixTime
#
#
# 					ijDiffPosInv = ijDistSqInv ** 0.5
#
#
# 					# Finally, we can compute the states
# 					stateVel = np.nansum(ijDiffVel, axis = 3)
# 					# print("stateVel: ", stateVel.shape)
#
# 					statePos = np.nansum(ijDiffPos, axis = 3)
#
# 					# print("statePos: ", statePos.shape)
#
# 					# statePosSqroot =np.nansum(posDiff *(const_ * (posDistSqInv ** 0.5)),
# 					# 						axis = 3)
# 					#
# 					# #   stateVel: batchSize[b] x 2 x nAgents
# 					# statePosFourth = np.nansum(posDiff * (const_ * (posDistSqInv ** 2)),
# 					# 						axis = 3)
# 					#   statePosFourth: batchSize[b] x 2 x nAgents
# 					# statePosSq = np.nansum(posDiff * posDistSqInv, axis = 3)
#
#
# 					statePosSqroot =np.nansum(const_ * ijDiffPos * ijDiffPosInv,
# 											axis = 3)
# 					statePosSq = np.nansum(ijDiffPos * ijDistSqInv, axis = 3)
#
# 					#   stateVel: batchSize[b] x 2 x nAgents
# 					statePosFourth = np.nansum(const_ * ijDiffPos * ijDiffPosInv * ijDistSqInv,
# 											axis = 3)
#
# 					#   statePosFourth: batchSize[b] x 2 x nAgents
#
...
# 																	),
# 																   axis = 1)
# 					# print("T Statedffff", state.shape)
# 					  # batchSize[b] x 6 x nAgents
#
# 					if doPrint:
# 						# Sample percentage count
...
# 				# Now, we need to compute the differences, in velocities and in
# 				# positions, for each agent, for each time instante
# 				ijDiffPos, ijDistSq = self.computeDifferences(posBatch)
# 				#   posDiff: batchSize[b] x tSamples x 2 x nAgents x nAgents
# 				#   posDistSq: batchSize[b] x tSamples x nAgents x nAgents
# 				ijDiffVel, _ = self.computeDifferences(velBatch)
# 				#   velDiff: batchSize[b] x tSamples x 2 x nAgents x nAgents
#
# 				# Next, we need to get ride of all those places where there are
# 				# no neighborhoods. That is given by the nonzero elements of the
# 				# graph matrix.
# 				# print(" print(graphMatrixBatch[0])", graphMatrixBatch.shape)
# 				# print("bafore", graphMatrixBatch[0])
# 				# print("pos.dtype:", pos.dtype)
# 				# print("pzeroTolerance:", zeroTolerance)
#
# 				# print("posBatch: ", posBatch.shape)
# 				graphMatrixTime = self.computeCommunicationGraph(posBatch, self.commRadius, self.normalizeGraph,
# 			   doPrint = False)[:,0,:,:]
#
# 				# print("graphMatrixTime: ", graphMatrixTime.shape)
# 				# graphMatrixBatch = (np.abs(graphMatrixBatch) > zeroTolerance)\
# 				# 											 .astype(pos.dtype)
#
# 				# print(" print(graphMatrixBatch[0])", graphMatrixBatch[0])
#
#
#
# 				#   graphMatrix: batchSize[b] x tSamples x nAgents x nAgents
...
# 				#   graphMatrix:batchSize[b] x tSamples x 1 x nAgents x nAgents
#
# 				# print("graphMatrixBatch: ", graphMatrixBatch[0][0])
# 				# Then, we can get rid of non-neighbors
# 				ijDistSqInv =np.expand_dims(ijDistSqInv,2) \
# 														  * graphMatrixBatch
# 				ijDiffVel = ijDiffVel * graphMatrixBatch
# 				ijDiffPos = ijDiffPos * graphMatrixBatch
# 				ijDiffPosInv = ijDistSqInv ** 0.5
# 				# print("velDiff bb:", velDiff.shape)
#
#
# 				# Finally, we can compute the states
# 				stateVel = np.nansum(ijDiffVel, axis = 4)
# 				statePos = np.nansum(ijDiffPos, axis = 4)
#
#
# 				statePosSqroot =np.nansum(const_ * ijDiffPos * ijDiffPosInv,
# 										axis = 4)
# 				statePosSq = np.nansum(ijDiffPos * ijDistSqInv, axis = 4)
#
# 				#   stateVel: batchSize[b] x 2 x nAgents
# 				statePosFourth = np.nansum(const_ * ijDiffPos * ijDiffPosInv * ijDistSqInv,
# 										axis = 4)
...
# 				kernelScale = 1.
#
# 		# print("mat: ", kernelScale, kernelType, weighted)
#
# 		# The print for this one can be settled independently, if not, use the
# 		# default of the data object
# 		if 'doPrint' in kwargs.keys():
...
# 																		   =  np.Inf
#
# 					# print("commRadius ** 2: ", np.sum((distSq[0, t,:,:] < commRadius ** 2).astype(float)))
# 					# print("commRadius ** 2: ", commRadius ** 2)
#
# 					graphMatrixTime = (distSq < commRadius ** 2).astype(float)
# 					# # Now let's place zeros in all places whose distance is greater
# 					# # than the radius
# 					# # print("GL: ", graphMatrixTime)
# 					# graphMatrixTime[distSq > (commRadius ** 2)] = 0.
# 					# # print("GL1: ", graphMatrixTime)
# 					# # Set the diagonal elements to zero
# 					graphMatrixTime[:,\
# 									np.arange(0,nAgents),np.arange(0,nAgents)]\
# 																		   = 0.
# 					# # print("GL2: ", graphMatrixTime)
# 					#
# 					# # If it is unweighted, force all nonzero values to be 1
# 					# if not weighted:
# 					# 	graphMatrixTime = (graphMatrixTime > zeroTolerance)\
...
# 				# Apply the Kernel
# 				# if kernelType == 'gaussian':
# 				# 	# print("gaussian--------",kernelScale)
# 				# 	graphMatrixBatch = np.exp(-kernelScale * distSq)
# 				# else:
# 				# print("distSq[0]: ", distSq[0].shape)
# 				# print("distSq:", np.sum(distSq[0]))
# 				# print(np.sum((distSq[0] < commRadius ** 2).astype(float)))
#
#
#
# 				distSq[:,:,
# 								 np.arange(0,nAgents),np.arange(0,nAgents)] = np.Inf
#
#
# 				# print(np.sum((distSq[0] < commRadius ** 2).astype(float)))
# 				graphMatrixBatch = (distSq < commRadius ** 2).astype(float)
# 				# print("distSq: ", distSq.shape)
# 				# print("distSq: ", distSq[0])
# 				# print("graphMatrixBatch: ", graphMatrixBatch[0])
#
# 				# Now let's place zeros in all places whose distance is greater
# 				# than the radius
# 				# graphMatrixBatch[distSq > (commRadius ** 2)] = 0.
...
# 				# 	graphMatrixBatch = (graphMatrixBatch > zeroTolerance)\
# 				# 										  .astype(distSq.dtype)
# 					# print("graphMatrixBatch Un weight: ", graphMatrixBatch[0])
# 				# print("normalizeGraph: ", normalizeGraph)
# 				if normalizeGraph:
# 					isSymmetric = np.allclose(graphMatrixBatch,
# 											  np.transpose(graphMatrixBatch,
# 															axes = [0,1,3,2]))
...
# 					graphMatrixBatch = graphMatrixBatch / maxEigenvalue
#
# 					# print("graphMatrixBatch normalize: ", graphMatrixBatch[0])
#
# 				# Store
# 				graphMatrix[batchIndex[b]:batchIndex[b+1]] = graphMatrixBatch
#
...
# 			# Erase the percentage
# 			print('\b \b' * 4, end = '', flush = True)
# 		# print("In commute Graph:", graphMatrix.shape)
# 		return graphMatrix
#
# 	def getData(self, name, samplesType, *args):
#
...
# 		return thisData
#
# 	def evaluate(self, pos = None, vel = None, accel = None, initVel = None,
# 				 samplingTime = None):
#
# 		# It is optional to add a different sampling time, if not, it uses
# 		# the internal one
...
# 		#logmetrics\
# 		if pos is not None:
# 			# print("pose: ", pos.shape)
# 			# print("pose: ",pos[:, -1, :, : ].reshape(2, nAgents))
# 			stats = {}
# 			last_pos = pos[:, -1, :, : ].reshape(nAgents,2)
# 			last_vel = vel[:, -1, :, : ].reshape(nAgents,2)
#
#
# 			area  = self.clustermetric.compute(last_pos, self.DEAD_agents)
# 			# print("area: ", area)
# 			n_clusters = self.n_clustermetric.compute(last_pos)
# 			avg = self.areametric.compute(last_pos)
# 			stats['area'] = area
# 			stats['AvgdAA'] = avg[0]
# 			stats['AvgdAB'] = avg[1]
# 			stats['n_clusters'] = n_clusters
# 			stats['vel_diffs'] = np.mean(np.sqrt(np.sum(np.power(last_vel - np.mean(last_vel, axis=0), 2), axis=1)))
#
#
# 			m = np.array(abs(last_pos))
# 			stats['vel_mean'] = m[:,0].mean()
#
#
# 			self.xij = np.subtract(np.repeat(last_pos[:,0], nAgents).reshape(nAgents, nAgents), last_pos[:,0])
# 			self.yij = np.subtract(np.repeat(last_pos[:,1], nAgents).reshape(nAgents, nAgents), last_pos[:,1])
#
# 			# Relative velocity among all pairs [v(j:2) - v(i:2)].
# 			self.vxij = np.subtract(np.repeat(last_vel[:,0], nAgents).reshape(nAgents, nAgents),last_vel[:,0])
# 			self.vyij = np.subtract(np.repeat(last_vel[:,1], nAgents).reshape(nAgents, nAgents), last_vel[:,1])
#
# 			# Relative distance among all pairs.
# 			self.dsqr = self.xij**2 + self.yij**2 #r2
# 			self.dist = np.sqrt(self.dsqr)
#
# 			np.fill_diagonal(self.dist, np.Inf)
#
# 			# print(self.dsqr)
#
# 			stats['min_dists'] = np.min(np.sqrt(self.dist ))
# 			# print(n_clusters, AvgdAA,AvgdAB)
#
# 		# And now that we have the velocities, we can compute the cost
# 		if 'torch' in repr(vel.dtype):
# 			# Average velocity for time t, averaged across agents
...
#
# 	def computeTrajectory(self, initPos, initVel, duration, **kwargs):
# 		# print("in compute traj segregation")
# 		# Check initPos is of shape batchSize x 2 x nAgents
# 		assert len(initPos.shape) == 3
# 		batchSize = initPos.shape[0]
# 		assert initPos.shape[1]
...
# 		if useArchit:
# 			accel = np.zeros((batchSize, tSamples, 2, nAgents), dtype=np.float)
# 			state = np.zeros((batchSize, tSamples, self.nstates, nAgents), dtype=np.float)
# 			graph = np.zeros((batchSize, tSamples, nAgents, nAgents),
# 							 dtype = np.float)
#
# 		# Assign the initial positions and velocities
...
# 				thisVel = np.expand_dims(vel[:,t-1,:,:], 1)
# 				# Compute state
# 				thisState = self.computeStates(thisPos, thisVel, thisGraph, self.const,
# 											   doPrint = False)
# 				# Save state
# 				state[:,t-1,:,:] = thisState.squeeze(1)
#
...
# 		thisPos = np.expand_dims(pos[:,-1,:,:], 1)
# 		thisGraph = self.computeCommunicationGraph(thisPos, self.commRadius,
# 													self.normalizeGraph, doPrint = False)
# 		graph[:,-1,:,:] = thisGraph.squeeze(1)
# 		#   State
# 		thisVel = np.expand_dims(vel[:,-1,:,:], 1)
# 		thisState = self.computeStates(thisPos, thisVel, thisGraph, self.const,
...
#
# 	def computeOptimalTrajectory(self, initPos, initVel, duration,
# 								 samplingTime, repelDist, alpha,
# 								 const_,
# 								 accelMax = 100.):
#
# 		# print("Const_", const_ )
# 		# print("Const_", const_.shape)
# 		# const_ = np.expand_dims(const_, 0)
# 		# const_ = np.expand_dims(const_, 0)
# 		# The optimal trajectory is given by
# 		# u_{i} = - \sum_{j=1}^{N} (v_{i} - v_{j})
# 		#         + 2 \sum_{j=1}^{N} (r_{i} - r_{j}) *
...
# 			# ##     Apply the mask to the relevant differences
# 			# ijDiffPos = ijDiffPos * np.expand_dims(repelMask, 1)
# 			# print("ijDiffPos BEfore: ", ijDiffPos.shape)
# 			# ijDiffPos = np.expand_dims(ijDiffPos, 1)
#
# 			# print("ijDiffPos: ", ijDiffPos.shape)
#
#
# 			# const_ = np.expand_dims(const_, 0)
#
...
#
# 			accel[:,t-1,:,:] = \
# 					-np.nansum(ijDiffVel, axis = 3) \
# 					+alpha* np.nansum(-ijDiffPos +
# 					ijDiffPos * ( const_ * ijDiffPosInv -
# 					ijDistSqInv +  const_ * (ijDistSqInv ** 2) ),
# 							   axis = 3) -vel[:,t-1,:,:]
#
# 			# Finally, note that if the agents are too close together, the
# 			# acceleration will be very big to get them as far apart as
...
# 			vel[:,t,:,:] = accel[:,t-1,:,:] * samplingTime + vel[:,t-1,:,:]
#
# 			# last_pos = pos[:, t, :, : ].reshape(nSamples, nAgents,2)
# 			#
# 			# for i in range(last_pos.shape[0]):
# 			#     n_clusters = self.n_clustermetric.compute(last_pos[i])
# 			#     print(i, n_clusters)
# 			#
# 			# print("thisAccel:", thisAccel.shape)
#
#
# 			if self.doPrint:
# 				# Sample percentage count
...
#
#
# 	def computeLocalOptimalTrajectory(self, initPos, initVel, duration,
# 								 samplingTime, repelDist, alpha,
# 								 const_, graphMatrix,
# 								 accelMax = 100.):
#
...
# 		accel = np.zeros((nSamples, tSamples, 2, nAgents))
#
# 		# print("initPos: ", initPos[0])
# 		# print("initVel: ", initVel[0])
#
# 		# Initial settings
# 		pos[:,0,:,:] = initPos
# 		vel[:,0,:,:] = initVel
...
# 			# Compute the optimal acceleration
# 			#   Compute the distance between all elements (positions)
# 			# print("pos[:,t-1,:,:]  before :", pos[0,t-1,:,:])
# 			ijDiffPos, ijDistSq = self.computeDifferences(pos[:,t-1,:,:])
#
# 			#       ijDiffPos: nSamples x 2 x nAgents x nAgents
# 			#       ijDistSq:  nSamples x nAgents x nAgents
...
# 			ijDiffVel, _ = self.computeDifferences(vel[:,t-1,:,:])
#
# 			graphMatrixTime = self.computeCommunicationGraph(np.expand_dims(pos[:,t-1,:,:], 1), self.commRadius, self.normalizeGraph,
# 		   doPrint = False)[:,0,:,:]
#
#
# 			#       Compute the constant (1/||r_ij||^4 + 1/||r_ij||^2)
...
# 			ijDistSqInv =np.expand_dims(ijDistSqInv,1) \
# 													  * graphMatrixTime
# 			# print("ijDiffVel:", ijDiffVel[0], ijDiffVel.shape)
#
# 			ijDiffVel = ijDiffVel * graphMatrixTime
# 			ijDiffPos = ijDiffPos * graphMatrixTime
#
# 			# print("ijDiffVel 2:", ijDiffVel[0], ijDiffVel.shape)
# 			# ||r_ij||
# 			ijDiffPosInv = ijDistSqInv ** 0.5
#
#
...
# 			# 		-np.nansum(ijDiffVel, axis = 3) \
# 			# 		-alpha* np.nansum(ijDiffPos
# 			# 		- (const_ * ijDiffPos * ijDiffPosInv)
# 			# 		+ (ijDiffPos * ijDistSqInv)
# 			# 		- (const_ * ijDiffPos * ijDiffPosInv * ijDistSqInv), axis = 3) \
# 			# 		- vel[:,t-1,:,:]
#
# 			accel[:,t-1,:,:] = \
# 					-np.nansum(ijDiffVel, axis = 3) \
...
# 			accel[:,t-1,:,:] = thisAccel
#
# 			# print("Clip Accel:", t-1,  accel[0,t-1,:,:])
# 			# if t >2:
# 			# 	break
#
# 			# Update the values
...
#
# 			#   Update the position
# 			pos[:,t,:,:] = accel[:,t-1,:,:] * (samplingTime ** 2) * 0.5 + \
# 								 vel[:,t-1,:,:] * samplingTime + pos[:,t-1,:,:]
#
# 			vel[:,t,:,:] = accel[:,t-1,:,:] * samplingTime + vel[:,t-1,:,:]
#
# 			# print("Pos: ", pos[0,t,:,:].T)
# 			# print("Sum Pos: ", np.sum(pos[0,t,:,:].T))
# 			# print("vel: ", vel[0,t,:,:].T)
#
#
# 			if self.doPrint:
...
# 		return pos, vel, accel
#
# 	def make_gif(self, pos, filename, fps=100):
# 		images = []
#
# 		tSamples = pos.shape[0]
# 		nAgents = pos.shape[2]
#
# 		xMinMap = np.min(pos[:,0,:]) * 1.2
# 		xMaxMap = np.max(pos[:,0,:]) * 1.2
...
#
# 		# Create figure
# 		videoFig = plt.figure(figsize = (3,3),linewidth=10, edgecolor="#04253a")
# 		# videoFig.set_cmap('hot')
#
# 		# Set limits
# 		ax = videoFig.add_subplot(111, aspect='equal', autoscale_on=True)#, \
# 		# plt.axis('off')
# 		plt.rcParams["figure.autolayout"] = True
# 		plt.rcParams["axes.edgecolor"] = "black"
# 		plt.rcParams["axes.linewidth"] = 2.50
# 		# xlim=(xMinMap, xMaxMap), ylim=(yMinMap, yMaxMap))
# 		# ax.grid(color='gray', linestyle='-', linewidth=0.1)
#
# 		cmap_ = plt.get_cmap('hsv')
# 		colors = [cmap_(i) for i in np.linspace(0, 500/self.GROUPS, 500)]
#
# 		self.handler = []
# 		# thisPos= pos
# 		for i in range(self.GROUPS):
# 			plotAgents, = ax.plot([], [], 'o', c = colors[i], ms=4)
# 			start = int(math.floor((i) * self.nAgents/self.GROUPS))
# 			stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
# 			plotAgents.set_data(pos[0,0,start:stop], pos[0,1,start:stop])
# 			self.handler.append(plotAgents)
# 		# plt.ion()
# 		# plt.show()
#
# 		for t in range(tSamples):
# 			if t % 500 or t== tSamples-1:
# 				for i in range(self.GROUPS):
# 					start = int(math.floor((i) * self.nAgents/self.GROUPS))
# 					stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
# 					self.handler[i].set_xdata(pos[t,0,start:stop])
# 					self.handler[i].set_ydata(pos[t,1,start:stop])
#
# 				ax.relim()
# 				ax.autoscale()
# 				videoFig.canvas.draw()
# 				image = np.frombuffer(videoFig.canvas.tostring_rgb(), dtype='uint8')
# 				image  = image.reshape(videoFig.canvas.get_width_height()[::-1] + (3,))
# 				images.append(image)
#
#
# 				videoFig.canvas.flush_events()
#
# 		plt.close(fig=videoFig)
# 		imageio.mimsave(filename, images, fps=fps)
#
# 	def evalTraj(self, pos, vel):
# 		nSamples = pos.shape[0]
# 		tSamples = pos.shape[1]
# 		nAgents = pos.shape[3]
# 		stats = {}
#
# 		stats['area'] = np.zeros((nSamples, tSamples))
# 		stats['n_clusters'] = np.zeros((nSamples, tSamples))
# 		stats['vel_diffs'] = np.zeros((nSamples, tSamples))
# 		stats['vel_mean'] = np.zeros((nSamples, tSamples))
# 		stats['AvgdAA'] = np.zeros((nSamples, tSamples))
# 		stats['AvgdAB'] = np.zeros((nSamples, tSamples))
# 		print("pose: ", pos.shape)
#
# 		for i in range(nSamples):
# 			for j in range(tSamples):
# 				# if j % 500 or j==tSamples-1:
# 				last_pos = pos[i, j, :, : ].reshape(nAgents,2)
# 				last_vel = vel[i, j, :, : ].reshape(nAgents,2)
# 				# print(self.areametric.compute(pos[i, j, :, : ].reshape(nAgents,2)))
# 				stats['area'][i,j] = self.clustermetric.compute(last_pos, self.DEAD_agents)
# 				stats['n_clusters'][i,j] = self.n_clustermetric.compute(last_pos)
# 				stats['vel_diffs'][i,j]  = np.mean(np.sqrt(np.sum(np.power(last_vel - np.mean(last_vel, axis=0), 2), axis=1)))
# 				m = np.array(abs(last_pos))
# 				stats['vel_mean'][i,j]  = m[:,0].mean()
#
# 				avg = self.areametric.compute(last_pos)
#
# 				stats['AvgdAA'][i,j]  = avg[0]
# 				stats['AvgdAB'][i,j]  = avg[1]
#
#
# 		return stats
#
...
# 		# or circular (or some other shape) and the parameters respecting
# 		# that
# 		assert geometry == 'rectangular' or geometry == 'circular' or geometry=='mine'
# 		assert minDist * (1.+zeroTolerance) <= commRadius * (1.-zeroTolerance)
# 		# We use a zeroTolerance buffer zone, just in case
# 		minDist = minDist * (1. + zeroTolerance)
# 		commRadius = commRadius * (1. - zeroTolerance)
#
# 		if geometry=='mine':
# 			initPos = np.load('/home/oyindamola/Research/Current_GNN_Research/graph-neural-networks/alegnn/utils/segregate_initPos_Agent'+str(self.nAgents)+'_GR'+str(self.GROUPS)+'_CR'+str(self.commRadius)+'_Samples421.npy',allow_pickle=True)
# 			initVel = np.load('/home/oyindamola/Research/Current_GNN_Research/graph-neural-networks/alegnn/utils/segregate_initVel_Agent'+str(self.nAgents)+'_GR'+str(self.GROUPS)+'_CR'+str(self.commRadius)+'_Samples421.npy',allow_pickle=True)
#
# 			# print(initPos[0])
# 			# print(initPos[0].T)
# 		# If there are other keys in the kwargs argument, they will just be
...
# 														 doPrint = False)
#
# 			# print("graphMatrix unsqueeze: ", graphMatrix.shape)
# 			graphMatrix = graphMatrix.squeeze(1) # nSamples x nAgents x nAgents
# 			# print("graphMatrix squeeze: ", graphMatrix.shape)
# 			#   Binarize the matrix
# 			graphMatrix = (np.abs(graphMatrix) > zeroTolerance)\
# 															 .astype(initPos.dtype)
#
...
# 				showVideoSpeed = False
#
# 		if 'videoNum' in kwargs.keys():
# 			videoNum = kwargs['videoNum']
# 		else:
# 			videoNum = ""
#
# 		if 'vel' in kwargs.keys():
...
# 			os.mkdir(saveDir)
#
# 		videoName = 'sampleTrajectory'+videoNum
#
# 		xMinMap = np.min(pos[:,:,0,:]) * 1.2
# 		xMaxMap = np.max(pos[:,:,0,:]) * 1.2
# 		yMinMap = np.min(pos[:,:,1,:]) * 1.2
...
# 		videoMetadata = dict(title = 'Sample Trajectory'+videoNum, artist = 'Segregation',
# 							 comment='Segregation example')
#
# 		videoWriter = FFMpegWriter(fps = videoSpeed/self.samplingTime,
# 								   metadata = videoMetadata)
#
...
# 				start = int(math.floor((i) * self.nAgents/self.GROUPS))
# 				stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
# 				plotAgents.set_data(thisPos[0,0,start:stop], thisPos[0,1,start:stop])
# 				self.handler.append(plotAgents)
# 			# plt.ion()
# 			# plt.show()
#
...
# 						# Plot the agents
# 						# Plot the agents
# 						# print("fa.sfasdlfsaml")
# 						# plotAgents.set_data(thisPos[t,0,:], thisPos[t,1,:])
# 						for i in range(self.GROUPS):
# 							start = int(math.floor((i) * self.nAgents/self.GROUPS))
# 							stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
# 							self.handler[i].set_xdata(thisPos[t,0,start:stop])
# 							self.handler[i].set_ydata(thisPos[t,1,start:stop])
# 						# plotAgents.set_data(thisPos[t,0,:], thisPos[t,1,:])
# 						ax.relim()
# 						ax.autoscale()
# 						videoFig.canvas.draw()
...
class Segregation(_data):
	...
	def __init__(self, nAgents,nstates, alpha, dAA, dAB, GROUPS, commRadius,
				repelDist, clipping,
				 ...
				 accelMax = 10.,
				 normalizeGraph = False, doPrint = True,
				 dataType = np.float64, device = 'cpu', data_=None):
		...
		self.dAB    = dAB; # % distance among distinct types.
		self.GROUPS = GROUPS;    #% number of groups.
		# self.WORLD  = 30;   #% world size.
		self.clipping = clipping
		self.DEAD_agents = []

		self.goal = np.random.uniform(-1, 1, size=(2,))
		self.goal = self.goal.reshape(1,2,1)
		self.nstates = nstates

		self.nAgents = nAgents

		self.clustermetric = ClusterMetric(self.GROUPS, self.nAgents)
		self.n_clustermetric = NClusterMetric(self.GROUPS, self.nAgents, self.dAB)
		self.areametric = AverageMetric(self.GROUPS, self.nAgents)

		self.group_assignments = np.zeros(self.nAgents)

		for i in range(self.GROUPS):
			start = int(math.floor((i) * self.nAgents/self.GROUPS))
			stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
			self.group_assignments[start:stop] = i
		...
		initPosAll, initVelAll = self.computeInitialPositions(
										  ...
										  geometry = self.initGeometry,
										  xMaxInitVel = self.initVelValue,
										  yMaxInitVel = self.initVelValue, data_=data_
															  )
		...
		self.state = {}
		self.localposAll = localposAll
		self.localvelAll = localvelAll

		# np.save('../examples/LocalPos_'+str(self.nAgents)+'_'+str(self.GROUPS)+'_'+str(self.alpha), self.localposAll)
		# np.save('../examples/LocalVel_'+str(self.nAgents)+'_'+str(self.GROUPS)+'_'+str(self.alpha), self.localvelAll)
		# np.save('../examples/GlobalPos_'+str(self.nAgents)+'_'+str(self.GROUPS)+'_'+str(self.alpha), self.posAll)
		# np.save('../examples/GlobalVel_'+str(self.nAgents)+'_'+str(self.GROUPS)+'_'+str(self.alpha), self.velAll)
		...
		print("test s: ", startSample)
		print("test st: ", endSample)
		self.samples['test']['signals']=stateAll[startSample:endSample].copy()
		self.samples['test']['targets']=accelAll[startSample:endSample].copy()
		self.initPos['test'] = initPosAll[startSample:endSample]
		self.initVel['test'] = initVelAll[startSample:endSample]
	...
	def computeStates(self, pos, vel, graphMatrix, const_,**kwargs):
		...
		for b in range(nBatches):
			...
			if tSamples > maxTimeSamples:
				...
				# For each time instant
				for t in range(tSamples):
					# print("it came here i checked it")
				   ...
				   doPrint = False)[:,0,:,:]
					...
					offset = self.goal - posBatch[:,t,:,:]
					dist_to_goal = np.linalg.norm(offset, axis=1)

					adj_goal = np.expand_dims((dist_to_goal < self.commRadius**2).astype(float), axis=1)

					offset_c = adj_goal*offset
					# print("k: ", offset_c.shape)
					# print("j: ", posBatch[:,t,:,:].shape)
				...
				# print("ps: ", posBatch.shape)
				offset = self.goal - posBatch
				# print("of: ", offset.shape)
				dist_to_goal = np.linalg.norm(offset, axis=(1,2))
				# print("dis: ", dist_to_goal.shape)

				adj_goal = np.expand_dims((dist_to_goal < self.commRadius**2).astype(float), axis=1)
				# print("adj_goal: ", adj_goal.shape)
				offset_c = adj_goal*offset

				# print("ofc: ", offset_c.shape)
			   ...
			   doPrint = False)[:,0,:,:]
				...
				if self.nstates == 10:
					...
					# print(state[batchIndex[b]:batchIndex[b+1]].shape)
				#   state: batchSize[b] x tSamples x 6 x nAgents
	...
	def evaluate_all(self, pos = None, vel = None, accel = None, initVel = None,
				 samplingTime = None):
		...
		if pos is not None:
			...
			for i in range(tSamples):
				...
				last_pos = pos[0, i, :, : ].transpose(1,0)
				last_vel = vel[0, i, :, : ].transpose(1,0)




				area  = self.clustermetric.compute(last_pos[:,0:2], self.DEAD_agents)
				# print("area: ", area)
				n_clusters = self.n_clustermetric.compute(last_pos[:,0:2])
				avg = self.areametric.compute(last_pos[:,0:2])
				stats['area'] = area
				stats['AvgdAA'] = avg[0]
				stats['AvgdAB'] = avg[1]
				stats['n_clusters'] = n_clusters
				...
				inter_group, intra_group = self.group_distance(last_pos, self.group_assignments, intergroup_distance=self.dAB, intragroup_distance=self.dAA)

				stats['inter_group'] = inter_group
				stats['intra_group'] = intra_group
				stats_.append(stats)


		return stats_

	def group_distance(self, robot_positions, group_assignments, intergroup_distance=1, intragroup_distance=0.5):
		num_robots = robot_positions.shape[0]
		num_groups = len(np.unique(group_assignments))
		...
		for i in range(num_robots):
			for j in range(i+1, num_robots):
				...
				if group_assignments[i] == group_assignments[j]:
					# Robots are in the same group
					inter_group.append(distance)
				else:
					# Robots are in different groups
					intra_group.append(distance)

		return np.mean(inter_group), np.mean(intra_group)
	...
	def computeTrajectory(self, initPos, initVel, duration, **kwargs):
		...
		for t in range(1, tSamples):
			...
			if useArchit:
				...
				# s_ = np.expand_dims(state[:,t-1,:,:], axis=1)
				# g_ = np.expand_dims(graph[:,t-1,:,:], axis=1)
				# x = torch.tensor(s_, device = architDevice)
				# S = torch.tensor(g_, device = architDevice)
				# print("x: ", x.shape)
				# print("S: ", S.shape)
				with torch.no_grad():
				...
				# Now that we have computed the acceleration, we only care
				# about the last element in time
				thisAccel = thisAccel.cpu().numpy()[:,-1,:,:] #- 0.01*thisVel
...
#
		...
		for t in range(1,tSamples):
			...
			offset = self.goal - pos[:, t-1, :, :]
			distance = np.linalg.norm(offset[:], axis=(1,2))
			target_speed = accelMax * np.minimum(1, distance/20 )
			target_velocity = target_speed.reshape(nSamples,1,1) * np.divide(offset,distance.reshape(nSamples,1,1))

			goal_accel = target_velocity - vel[:, t-1, :, :]
			...
			accel[:,t-1,:,:] = \
					...
					ijDiffPos * ( const_ * ijDiffPosInv -
					ijDistSqInv +  const_ * (ijDistSqInv ** 2) ),
							   axis = 3) - vel[:,t-1,:,:] #+ self.alpha * goal_accel
	...
	def make_gif(self, pos, filename, fps=100):
		...
		# Set limits
		# ax = videoFig.add_subplot(111, aspect='equal', autoscale_on=True)#, \
		ax = videoFig.add_subplot(111, aspect='equal',xlim=(-1.0 * 10, 1.0 * 10), ylim=(-1.0 * 10, 1.0 * 10))
		# plt.axis('on')
		ax.grid(color='gray', linestyle='-', linewidth=0.1)
		plt.rcParams["figure.autolayout"] = True
		plt.rcParams["axes.edgecolor"] = "black"
	...
	def perform_logging(self, itr, logs, logger, stats, print_data=True):
		if logs is None:
			logs = OrderedDict()
		...
		if stats is not None:
			logs['n_clusters'] = stats['n_clusters']
			logs["AvgdAA"] = stats['AvgdAA']
			logs["AvgdAB"] = stats['AvgdAB']
			logs['area'] = stats['area']
			logs['vel_mean'] = stats['vel_mean']
			logs['min_dists'] = stats['min_dists']
			logs['vel_diffs'] = stats['vel_diffs']

		logger.dump_tabular(itr, logs, print_data)
		# perform the logging
	...
	def computeInitialPositions(self, nAgents, nSamples, commRadius,
								minDist = 0.1, geometry = 'rectangular', data_=None,
								**kwargs):
...
class Segregation3D(_data):
	...
	def __init__(self, nAgents, nstates, alpha, dAA, dAB, GROUPS, commRadius,
		...
		#%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
		self.alpha  = alpha; # % control gain.
		self.dAA    = dAA;  #% sdistance among same types.
		self.dAB    = dAB; # % distance among distinct types.
		self.GROUPS = GROUPS;    #% number of groups.
		# self.WORLD  = 30;   #% world size.
		self.clipping = clipping
		...
		self.goal = np.array([0,0,10])
		self.goal = self.goal.reshape(1,3,1)
		self.nstates = nstates
		...
		# 								accelMax = self.accelMax)
		#
		# self.localposAll, self.localvelAll = localposAll, localvelAll
	...
	def computeLocalOptimalTrajectory(self, initPos, initVel, duration,
		...
		assert len(initPos.shape) == len(initVel.shape) == 3
		nSamples = initPos.shape[0]
		assert initPos.shape[1] == initVel.shape[1] == 3
		nAgents = initPos.shape[2]
		assert initVel.shape[0] == nSamples
		assert initVel.shape[2] == nAgents
		...
		# print("tSamples: ", tSamples)
		# Create arrays to store the trajectory
		pos = np.zeros((nSamples, tSamples, 3, nAgents))
		vel = np.zeros((nSamples, tSamples, 3, nAgents))
		accel = np.zeros((nSamples, tSamples, 3, nAgents))
	...
	def computeStates(self, pos, vel, graphMatrix, const_,**kwargs):
		...
		for b in range(nBatches):
			...
			if tSamples > maxTimeSamples:
				...
				adj_goal = np.expand_dims((dist_to_goal < self.commRadius**2).astype(float), axis=1)
				# print("adj_goal: ", adj_goal.shape)
				offset_c = np.expand_dims(adj_goal,axis=2)*offset
			   ...
			   doPrint = False)[:,0,:,:]
				...
				if self.nstates == 15:
					...
					# print(stateVel.shape,
					# 				statePos.shape,
					# 				statePosFourth.shape,
					# 				statePosSq.shape,
					# 				statePosSqroot.shape,
					# 				velBatch.shape,
					# 				offset_c.shape)
					# print("---------------------------------------------")
					state[batchIndex[b]:batchIndex[b+1]] = \
	...
	def evaluate(self, pos = None, vel = None, accel = None, initVel = None,
		...
		if vel is not None:
			...
			if 'torch' in repr(accel.dtype):
				...
				assert 'torch' in repr(initVel.dtype)
				# Create the tensor to save the velocity trajectory
				vel = torch.zeros(nSamples,tSamples,3,nAgents,
								  dtype = accel.dtype, device = accel.device)
				# Add the initial velocity
				vel[:,0,:,:] = initVel.clone().detach()
			else:
		...
		if pos is not None:
			...
			# print("pose: ",pos[:, -1, :, : ].reshape(2, nAgents))
			stats = {}
			last_pos = pos[:, -1, :, : ].reshape(nAgents,3)
			last_vel = vel[:, -1, :, : ].reshape(nAgents,3)
			...
			self.xij = np.subtract(np.repeat(last_pos[:,0], nAgents).reshape(nAgents, nAgents), last_pos[:,0])
			self.yij = np.subtract(np.repeat(last_pos[:,1], nAgents).reshape(nAgents, nAgents), last_pos[:,1])
			self.zij = np.subtract(np.repeat(last_pos[:,2], nAgents).reshape(nAgents, nAgents), last_pos[:,2])

			# Relative velocity among all pairs [v(j:2) - v(i:2)].
			self.vxij = np.subtract(np.repeat(last_vel[:,0], nAgents).reshape(nAgents, nAgents),last_vel[:,0])
			self.vyij = np.subtract(np.repeat(last_vel[:,1], nAgents).reshape(nAgents, nAgents), last_vel[:,1])
			self.vzij = np.subtract(np.repeat(last_vel[:,2], nAgents).reshape(nAgents, nAgents), last_vel[:,2])

			# Relative distance among all pairs.
			self.dsqr = self.xij**2 + self.yij**2 + self.zij**2 #r2
			self.dist = np.sqrt(self.dsqr)
	...
	def evaluate_all(self, pos = None, vel = None, accel = None, initVel = None,
		...
		if pos is not None:
			...
			for i in range(tSamples):
				...
				last_pos = pos[0, i, :, : ].transpose(1,0)
				last_vel = vel[0, i, :, : ].transpose(1,0)
				area  = self.clustermetric.compute(last_pos[:,0:3], self.DEAD_agents)
				...
				# 	area  = self.clustermetric.compute(last_pos[:,0:3], self.DEAD_agents)
				# # print("area: ", area)
				n_clusters = self.n_clustermetric.compute(last_pos[:,0:3])
				avg = self.areametric.compute(last_pos[:,0:3])
				stats['area'] = area
				stats['AvgdAA'] = avg[0]
				stats['AvgdAB'] = avg[1]
				stats['n_clusters'] = n_clusters
	...
	def computeTrajectory(self, initPos, initVel, duration, **kwargs):
		...
		# Now create the outputs that will be filled afterwards
		pos = np.zeros((batchSize, tSamples, 3, nAgents), dtype = np.float)
		vel = np.zeros((batchSize, tSamples, 3, nAgents), dtype = np.float)
		if useArchit:
			accel = np.zeros((batchSize, tSamples, 3, nAgents), dtype=np.float)
			state = np.zeros((batchSize, tSamples, self.nstates, nAgents), dtype=np.float)
			graph = np.zeros((batchSize, tSamples, nAgents, nAgents),
							 dtype = np.float)
		...
		for t in range(1, tSamples):
			...
			if useArchit:
				...
				#   to now, because when we do the communication exchanges,
				#   it involves past times.
				# print(state[:,0:t,:,:].shape)
				# print("g: ", state[:,t,:,:].shape)
				# s_ = np.expand_dims(state[:,t,:,:], axis=1)
				# g_ = np.expand_dims(graph[:,t,:,:], axis=1)
				x = torch.tensor(state[:,0:t,:,:], device = architDevice)
				S = torch.tensor(graph[:,0:t,:,:], device = architDevice)
				...
				# Now that we have computed the acceleration, we only care
				# about the last element in time
				# print(thisVel.shape, "dddddddddddddddddddddddd")

				thisAccel = thisAccel.cpu().numpy()[:,-1,:,:]-0.3*vel[:,t-1,:,:]
				...
				# print("accele", thisAccel.shape, thisAccel)
		...
		# print("pose", pos)
		# And return it
		np.save('../examples/Testsegregate3D_Action_Agent'+str(self.nAgents)+'_GR'+str(self.GROUPS)+'_CR'+str(self.commRadius)+"_Samples"+str(1),  accel)
	...
	def computeDifferences(self, u):
		...
		# Takes as input a tensor of shape
		#   nSamples x tSamples x 3 x nAgents
		# or of shape
		#   nSamples x 3 x nAgents
		# And returns the elementwise difference u_i - u_j of shape
		#   nSamples (x tSamples) x 3 x nAgents x nAgents
		# And the distance squared ||u_i - u_j||^2 of shape
		#   nSamples (x tSamples) x nAgents x nAgents
		...
		# Second, for axis z
		uCol_z = u[:,:,2,:].reshape((nSamples, tSamples, nAgents, 1))
		uRow_z = u[:,:,2,:].reshape((nSamples, tSamples, 1, nAgents))
		uDiff_z = uCol_z - uRow_z # nSamples x tSamples x nAgents x nAgents


		# Third, compute the distance tensor of shape
		#   nSamples x tSamples x nAgents x nAgents
		uDistSq = uDiff_x ** 2 + uDiff_y ** 2 + uDiff_z ** 2
		# Finally, concatenate to obtain the tensor of differences
		#   Add the extra dimension in the position
		uDiff_x = np.expand_dims(uDiff_x, 2)
		uDiff_y = np.expand_dims(uDiff_y, 2)
		uDiff_z = np.expand_dims(uDiff_z, 2)
		#   And concatenate them
		uDiff = np.concatenate((uDiff_x, uDiff_y, uDiff_z), 2)
		#   nSamples x tSamples x 2 x nAgents x nAgents
...
#
	...
	def make_gif(self, pos, filename, fps=100):
		...
		yMinMap = np.min(pos[:,1,:]) * 1.2
		yMaxMap = np.max(pos[:,1,:]) * 1.2
		zMinMap = np.min(pos[:,2,:]) * 1.2
		zMaxMap = np.max(pos[:,2,:]) * 1.2

		# Create figure
		videoFig = plt.figure(figsize = (10,10))
		# videoFig.set_cmap('hot')
		...
		# Set limits
		# ax = videoFig.add_subplot(111, aspect='equal', autoscale_on=True)#, \
		# ax = videoFig.add_subplot(111, projection='3d', aspect='auto', autoscale_on=True,xlim=(-1.0 * self.WORLD, 1.0 * self.WORLD), ylim=(-1.0 * self.WORLD, 1.0 * self.WORLD),zlim=(-1.0 * self.WORLD, 1.0 * self.WORLD))
		...
		ax.grid()
		ax.set_xlim(-self.WORLD,self.WORLD)
		ax.set_ylim(-self.WORLD,self.WORLD)
		ax.set_zlim(-self.WORLD*1.5,self.WORLD*1.5)
		...
		for axis in [ax.xaxis, ax.yaxis, ax.zaxis]:
			...
			axis._axinfo['grid']['color'] =  "lightgray"
		...
		# thisPos= pos
		for i in range(self.GROUPS):
			plotAgents, = ax.plot3D([], [], [], 'o', color=colors[i], ms=12,markeredgecolor='black',pickradius=5)
			start = int(math.floor((i) * self.nAgents/self.GROUPS))
			stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
			plotAgents.set_data_3d(pos[0,0,start:stop], pos[0,1,start:stop], pos[0,2,start:stop])
			self.handler.append(plotAgents)
		# plt.ion()
		# plt.show()
		...
		for t in range(tSamples):
			if t % 10==0 or t== tSamples-1:
				for i in range(self.GROUPS):
					...
					self.handler[i].set_xdata(pos[t,0,start:stop])
					self.handler[i].set_ydata(pos[t,1,start:stop])
					self.handler[i].set_3d_properties(pos[t,2,start:stop])
				...
				# ax.set_xlim([xmin * self.WORLD, xmax * self.WORLD])
				# ax.set_ylim([ymin * self.WORLD, ymax * self.WORLD])
				# ax.set_zlim([zmin * self.WORLD, zmax * self.WORLD])
	...
	def computeInitialPositions(self, nAgents, nSamples, commRadius,
		...
		if geometry=='mine':
			if data_ is not None:
				...
				initPos = np.load('/home/oyindamola/Research/Current_GNN_Research/graph-neural-networks/alegnn/utils/segregate3D_initPos_Agent'+str(self.nAgents)+'_GR'+str(self.GROUPS)+'_CR'+str(self.commRadius)+'_Samples421.npy',allow_pickle=True)
				initVel = np.load('/home/oyindamola/Research/Current_GNN_Research/graph-neural-networks/alegnn/utils/segregate3D_initVel_Agent'+str(self.nAgents)+'_GR'+str(self.GROUPS)+'_CR'+str(self.commRadius)+'_Samples421.npy',allow_pickle=True)
		...
		# circular.
			...
			# Same for the other axis
			if 'zMaxInitVel' in kwargs.keys():
				zMaxInitVel = kwargs['zMaxInitVel']
			else:
				zMaxInitVel = 3.
			...
			yInitVel = np.random.uniform(low = -yMaxInitVel, high = yMaxInitVel,
										 size = (nSamples, 1, nAgents))
			zInitVel = np.random.uniform(low = -zMaxInitVel, high = zMaxInitVel,
										 size = (nSamples, 1, nAgents))
			...
			yVelBias = np.random.uniform(low = -yMaxInitVel, high = yMaxInitVel,
										 size = (nSamples))
			zVelBias = np.random.uniform(low = -zMaxInitVel, high = zMaxInitVel,
										 size = (nSamples))
			# And concatenate them
			velBias = np.concatenate((xVelBias, yVelBias, zVelBias)).reshape((nSamples,3,1))
			initVel = np.concatenate((xInitVel, yInitVel, zInitVel), axis = 1) + velBias
		print("initPos: ", initPos.shape)
		print("initVel: ", initVel.shape)
	...
	def saveVideo(self, saveDir, pos, *args,
				  commGraph = None, if_gif=False, **kwargs):
		...
		for n in range(pos.shape[0]):
			...
			self.WORLD = 50
			# self.fig.subplots_adjust(left=0, right=1, bottom=0, top=1)
			# ax = videoFig.add_subplot(111,projection='3d', aspect='auto', autoscale_on=False, \
			# xlim=(-1.0 * self.WORLD, 1.0 * self.WORLD), ylim=(-1.0 * self.WORLD, 1.0 * self.WORLD), \
			# zlim=(-1.0 * self.WORLD, 1.0 * self.WORLD))

			ax = plt.axes(projection='3d')
			ax.grid()
			ax.set_xlim(-self.WORLD,self.WORLD)
			ax.set_ylim(-self.WORLD,self.WORLD)
			ax.set_zlim(-self.WORLD,self.WORLD)
			...
			# if showVideoSpeed:
			# 	plt.text(xMinMap, yMinMap, zMinMap, r'Speed: $%.2f$' % videoSpeed)


			self.handler = []
			for i in range(self.GROUPS):
				plotAgents, = ax.plot([], [], 'o', c = colors[i], ms=7)
				start = int(math.floor((i) * self.nAgents/self.GROUPS))
				stop = int(math.floor((i+1) * self.nAgents/self.GROUPS))
				plotAgents.set_data_3d(thisPos[0,0,start:stop], thisPos[0,1,start:stop], thisPos[0,2,start:stop])
				self.handler.append(plotAgents)
			# plt.ion()
			# plt.show()
				...
				for t in range(tSamples):
					...
					if t % 50==0 or t == tSamples-1:
						...
						for i in range(self.GROUPS):
							...
							self.handler[i].set_xdata(thisPos[t,0,start:stop])
							self.handler[i].set_ydata(thisPos[t,1,start:stop])
							self.handler[i].set_3d_properties(thisPos[t,2,start:stop])
						# plotAgents.set_data(thisPos[t,0,:], thisPos[t,1,:])
						# ax.relim()
						# ax.autoscale()
						# plt.savefig(os.path.join(saveDir,thisVideoName)+"test_rasterization.png", dpi=150)
						videoFig.canvas.draw()
						if if_gif:
							# print("YA GIF")
							image = np.frombuffer(videoFig.canvas.tostring_rgb(), dtype='uint8')
			...
			plt.close(fig=videoFig)
			if if_gif:
				imageio.mimsave(os.path.join(saveDir,thisVideoName[:-4])+'.gif', self.images, fps=100)
		...
		# 			plt.xlim((xMinMap, xMaxMap))
		# 			plt.ylim((yMinMap, yMaxMap))
		# 			plt.zlim((zMinMap, zMaxMap))
		# 			plt.axis('equal')
		#
		# 			# Plot the edges
		# 			plt.plot([thisPos[t,0,outEdge], thisPos[t,0,inEdge]],
		# 					 [thisPos[t,1,outEdge], thisPos[t,1,inEdge]],
		# 					  [thisPos[t,2,outEdge], thisPos[t,2,inEdge]],
		# 					 color = '#A8AAAF', linewidth = 0.75,
		# 					 scalex = False, scaley = False)
		#
		# 			# Plot the arrows
